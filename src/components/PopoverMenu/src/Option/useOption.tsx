    ...
    if (isValidElement(child)) {
      if (child.type === Telicon) {
        return React.cloneElement(child as ReactElement, {
          key: `telicon-${index}`,
          size: 'default',
          className: classNames('sds_popoverMenu_telicon', {
      ...
      if (child.type === Avatar) {
        return React.cloneElement(child as React.ReactElement, {
          key: `avatar-${index}`,
          size: 'small',
          className: classNames('sds_popoverMenu_avatar', {
            [(child as ReactElement).props.className as string]: (
              child as ReactElement
            ).props.className,
          }),
      ...
      if (React.Children.count((child as ReactElement).props.children) > 0) {
        //this is done if the telicon is not a direct child of the popoverMenuoption
